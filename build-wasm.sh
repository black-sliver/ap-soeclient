...
if [[ "$1" == "debug" ]] || [[ "$1" == "stat" ]]; then
  em++ --bind $SRC "src/games/$GAME_C" $INCLUDE_DIRS -DGAME_H="\"games/$GAME_H\"" $LIBS --shell-file ui/shell.html -o "$BUILD_DIR/$NAME.html" -fexceptions || exit 1
fi

# release build
if [[ "$1" != "debug" ]]; then
  em++ --bind $SRC "src/games/$GAME_C" $INCLUDE_DIRS -DGAME_H="\"games/$GAME_H\"" $LIBS --shell-file ui/shell.html -o "$BUILD_DIR/$NAME.min.html" -fexceptions -Oz -flto || exit 1
